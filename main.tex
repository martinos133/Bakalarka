...
\caption{Panel dynamických filtrov -- vstupy generované podľa konfigurácie kategórie (typ \texttt{TEXT}, \texttt{NUMBER}, \texttt{SELECT}, \texttt{MULTISELECT})}
\label{fig:filters}
\end{figure}
...
\textbf{Správa inzerátov:} Dashboard používateľa zobrazuje všetky jeho inzeráty vrátane ich aktuálneho stavu (\hlcode{PENDING}, \hlcode{ACTIVE}, \hlcode{ARCHIVED}) a štatistík zobrazení. Používateľ môže inzeráty editovať, deaktivovať alebo zmazať. Prehľadovú stránku profilu predajcu so zoznamom jeho aktívnych inzerátov dokumentuje obrázok~\ref{fig:profile-inzerat}.
...
\textbf{Platby a transakcie:} Sekcia platieb v používateľskej zóne agreguje finančné transakcie nad inzerátmi v dvoch perspektívach -- prijaté platby (pohľad vlastníka/\hlcode{owner}) a uskutočnené platby (pohľad záujemcu/\hlcode{renter}). Každá transakcia obsahuje výšku sumy, stav (\hlcode{PaymentStatus}), obdobie prenájmu a referenciu na konkrétny inzerát. Prehľadovú obrazovku zobrazuje obrázok~\ref{fig:platba-1}, detail jednotlivej platby s časovou osou stavov je dokumentovaný na obrázku~\ref{fig:platby-2}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{images/platba-1.png}
\caption{Modul platieb -- prehľad transakcií používateľa s filtrovaním podľa stavu (\texttt{PENDING}, \texttt{COMPLETED}, \texttt{FAILED}, \texttt{CANCELLED}, \texttt{REFUNDED})}
\label{fig:platba-1}
\end{figure}
