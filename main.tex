...
\title{Návrh a implementácia webovej inzerenčnej platformy}
\author{Martin Muha}
...
\noindent
V súčasnej digitálnej ére prechádza spôsob, akým spotrebitelia vyhľadávajú služby a prenájmy, zásadnou transformáciou \parencite{vallas2020platforms}. Tradičné formy inzercie sú nahrádzané interaktívnymi webovými platformami, ktoré umožňujú okamžitý prístup k informáciám v reálnom čase. Hlavnou motiváciou pre vývoj modernej inzertnej platformy je potreba konsolidovať rozptýlenú ponuku do jedného transparentného a bezpečného ekosystému. Moderný používateľ očakáva vysokú mieru flexibility a rýchlosti pri hľadaní odborných služieb či prenájmu, pričom kľúčovým faktorom úspechu je budovanie mena aplikácie, zvyšovanie jej návštevnosti a vznik takzvaného sieťového efektu \parencite{rochet2003platform}. Pri službách a prenájmoch zohráva geografická poloha kritickú úlohu, a preto implementácia pokročilých mapových rozhraní a lokalizačných služieb (v projekte riešené cez knižnicu \hlcode{Leaflet}) výrazne zvyšuje používateľský komfort \parencite{obe2017postgresql}. Okrem funkčnej vybavenosti je kľúčovým prvkom významu platformy aj jej jednoduchosť, intuitívne používateľské rozhranie a nízka bariéra vstupu \parencite{riva2022nextjs}. Jednou z najväčších výziev online inzercie je eliminácia podvodného správania -- platforma musí garantovať bezpečnosť prostredníctvom aktívnej moderácie, systému nahlasovania a prísnych pravidiel schvaľovania inzerátov, čím buduje nevyhnutnú dôveru medzi subjektmi v digitálnom priestore \parencite{hoffman2020websec, sundararajan2014peer}.

Súčasný digitálny trh so službami a prenájmom na Slovensku je príliš rozdrobený, čo komplikuje kontakt medzi tými, ktorí službu ponúkajú, a tými, ktorí ju hľadajú. Hlavným problémom je chýbajúca univerzálna platforma, ktorá by dokázala spojiť rôzne profesie a typy prenájmu na jednom mieste. Používatelia sú dnes nútení hľadať riešenia cez viaceré, často špecializované alebo neformálne kanály -- portál \hlcode{pretlak.com} sa zameriava takmer výhradne na IT, marketing a kreatívny priemysel, sociálne skupiny na Facebooku slúžia len úzkemu okruhu odborníkov a portál \hlcode{bazos.sk} je primárne určený na predaj tovaru. Z pohľadu bezpečnosti sú sociálne siete a fóra rizikové, pretože im chýbajú nástroje na kontrolu obsahu alebo overovanie osôb; prevádzkovatelia týchto skupín navyše nevedia sledovať, o čo majú ľudia skutočný záujem, pretože nemajú prístup k analytickým dátam.

Hlavným cieľom tejto bakalárskej práce je navrhnúť a vytvoriť modernú webovú platformu, ktorá slúži na uverejňovanie a správu inzerátov v oblasti služieb a prenájmu. Celý systém je navrhnutý tak, aby na jednom mieste prepojil bežných používateľov, poskytovateľov služieb a administrátorov, čím nahrádza súčasné neprehľadné hľadanie na sociálnych sieťach alebo všeobecných inzertných weboch. Konkrétne čiastkové úlohy zahŕňajú: vytvorenie stabilného a bezpečného backendu pomocou frameworku \hlcode{NestJS} s JWT autentifikáciou a databázou \hlcode{PostgreSQL}; vývoj troch samostatných \hlcode{Next.js} aplikácií prispôsobených konkrétnym skupinám používateľov (verejná platforma, používateľská zóna, administračný panel); implementáciu interaktívnej mapy cez \hlcode{Leaflet}; návrh systému dynamických filtrov a kategórií; a vytvorenie analytických nástrojov spolu s funkciami na moderovanie obsahu a nahlasovanie nevhodných inzerátov.

Práca je členená do siedmich kapitol. Prvá kapitola podrobuje analýze súčasný stav riešenej problematiky a porovnáva existujúce platformy. Druhá kapitola popisuje analýzu a návrh systému vrátane funkčných požiadaviek, databázového modelu a REST API rozhrania. Tretia kapitola sa venuje použitým technológiám (TypeScript, NestJS, Next.js, PostgreSQL, Prisma ORM, Docker). Štvrtá kapitola opisuje realizáciu a implementáciu jednotlivých modulov a klientskych aplikácií. Piata kapitola prezentuje testovanie systému na úrovni jednotkových testov, REST API, bezpečnostného testovania podľa OWASP Top Ten a manuálneho používateľského testovania. Šiesta kapitola diskutuje prínosy práce a možnosti ďalšieho rozšírenia.
...
\caption{Diagram použitia (Use Case) -- funkčné požiadavky a aktéri systému (Zdroj: Martin Muha.)}
...
\caption{Komponentová architektúra systému RentMe -- tri Next.js frontendové aplikácie komunikujú cez REST API s NestJS backendom; perzistencia je riešená cez Prisma ORM nad PostgreSQL, celý ekosystém je orchestrovaný cez Docker Compose (Zdroj: Martin Muha.)}
...
\caption{Tok zápisu dát po validácii: kontrolér, aplikačná služba, Prisma ORM a databáza PostgreSQL (Zdroj: Martin Muha.)}
...
\begin{figure}[p]
\centering
\resizebox{\textwidth}{!}{\input{diagrams/erd-database.tex}}
\caption{Entitno-relačný diagram (ERD) databázy -- jadrové entity, podporné tabuľky a ich relácie (Zdroj: Martin Muha.)}
...
\end{tabular}
\endgroup
\caption{Prehľad hlavných modulov API a základných ciest (Zdroj: Martin Muha.)}
...
\caption{Mapové zobrazenie inzerátov (Leaflet) -- markery agregované do clusterov pri väčšom priblížení a dynamické načítanie polôh z endpointu \texttt{GET /advertisements/map} (Zdroj: Martin Muha.)}
\label{fig:mapa-1}
\end{figure}
...
\caption{Interaktívny popup nad markerom v mape Leaflet -- náhľad inzerátu s fotografiou, názvom, cenou a odkazom na detail (Zdroj: Martin Muha.)}
\label{fig:mapa-2}
\end{figure}
...
\end{tabular}
\endgroup
\caption{Porovnanie PHP (Laravel) a Node.js/TypeScript ekosystému (Zdroj: Martin Muha.)}
...
\begin{figure}[htbp]
\centering
\resizebox{!}{0.65\textheight}{\input{diagrams/state-advertisement.tex}}
\caption{Stavový diagram životného cyklu inzerátu -- päť stavov enumerátora \texttt{AdvertisementStatus} a prechody medzi nimi; akcie vyvoláva buď používateľ (vytvorenie, odoslanie, deaktivácia) alebo administrátor (schválenie, zamietnutie) (Zdroj: Martin Muha.)}
...
\caption{Verejná platforma -- úvodná stránka s hero sekciou, hlavnou navigáciou a prezentáciou kľúčových kategórií (Zdroj: Martin Muha.)}
...
\caption{Verejná platforma -- prehľad aktívnych inzerátov so zoznamovým zobrazením a postrannými filtrami (Zdroj: Martin Muha.)}
...
\caption{Detail kategórie -- zoznam inzerátov v rámci zvolenej kategórie s hierarchickou navigáciou a SEO-optimalizovanou URL (Zdroj: Martin Muha.)}
...
\caption{Panel dynamických filtrov -- vstupy generované podľa konfigurácie kategórie (typy \texttt{TEXT}, \texttt{NUMBER}, \texttt{SELECT}, \texttt{MULTISELECT}, \texttt{BOOLEAN}, \texttt{DATE}, \texttt{RANGE}) (Zdroj: Martin Muha.)}
\label{fig:filters}
\end{figure}
...
\caption{Verejná platforma -- interaktívne mapové zobrazenie inzerátov implementované knižnicou Leaflet (Zdroj: Martin Muha.)}
...
\caption{Verejná platforma -- sekcia blogu a informačných stránok generovaných cez SSR z databázy (Zdroj: Martin Muha.)}
...
\caption{Sprievodca tvorbou inzerátu -- krok~1: výber typu ponuky (služba alebo prenájom) a hierarchický výber kategórie (Zdroj: Martin Muha.)}
\label{fig:inzerat-1}
\end{figure}
...
\caption{Sprievodca tvorbou inzerátu -- krok~2: zadanie základných údajov (názov, popis, cena) a dynamických špecifikácií podľa zvolenej kategórie (Zdroj: Martin Muha.)}
\label{fig:inzerat-2}
\end{figure}
...
\caption{Sprievodca tvorbou inzerátu -- krok~3: nahrávanie obrazovej dokumentácie a definícia balíčkov služieb (Zdroj: Martin Muha.)}
\label{fig:inzerat-3}
\end{figure}
...
\caption{Sprievodca tvorbou inzerátu -- krok~4: výber polohy na interaktívnej mape (Leaflet) a finálne odoslanie inzerátu na schválenie (Zdroj: Martin Muha.)}
\label{fig:inzerat-4}
\end{figure}
...
\caption{Verejný profil predajcu -- prehľad aktívnych inzerátov, kontaktné údaje a hodnotenia získané od ostatných používateľov platformy (Zdroj: Martin Muha.)}
...
\caption{Pridanie inzerátu -- horná časť formulára so základnými údajmi (typ ponuky, kategória, názov, popis, cena) a možnosťou nahratia fotografií (Zdroj: Martin Muha.)}
...
\caption{Pridanie inzerátu -- spodná časť formulára s dynamickými špecifikáciami, výberom polohy na mape Leaflet a tlačidlom na odoslanie do moderačnej fronty (Zdroj: Martin Muha.)}
...
\caption{Výber predajného balíka -- štyri úrovne \texttt{SellerPlan} (Štandard~--~0\,€/mesiac, Plus~--~8,99\,€/mesiac, RentMe Pro~--~16,99\,€/mesiac, Firma~--~39\,€/mesiac) s porovnaním funkcií a viditeľnosti inzerátov (Zdroj: Martin Muha.)}
\label{fig:platba-1}
\end{figure}
...
\caption{Modul platieb -- detail jednotlivej transakcie so sumou, obdobím prenájmu, referenciou na inzerát a časovou osou zmien stavu (Zdroj: Martin Muha.)}
\label{fig:platby-2}
\end{figure}
...
\caption{Prihlasovacia obrazovka administračného panela RentMe -- minimalistický dizajn s tmavou paletou a prémiovým zlatým akcentom (Zdroj: Martin Muha.)}
...
\caption{Hlavná nástenka admin panela -- bočná navigácia (Nástenky, Komunikácia, Moderácia, Správa, Obsah), rýchle akcie, agregované metriky a panel s nedávnymi konverzáciami (Zdroj: Martin Muha.)}
...
\caption{Štatistická sekcia admin panela -- časové rady inzerátov (zlatá krivka) a registrácií používateľov (modrá krivka) s prepínateľným obdobím (7 dní / 30 dní / 3 mesiace) a interaktívnym tooltipom pre detailné hodnoty v konkrétnom dni (Zdroj: Martin Muha.)}
...
\end{tabular}
\endgroup
\caption{Overené REST API scenáre platformy RentMe (Zdroj: Martin Muha.)}
...
\end{tabular}
\endgroup
\caption{Výsledky bezpečnostného testovania podľa OWASP Top Ten (Zdroj: Martin Muha.)}
...
\end{tabular}
\endgroup
\caption{Výsledky manuálneho používateľského testovania (Zdroj: Martin Muha.)}
...
\noindent
V rámci tejto bakalárskej práce som navrhol a implementoval modernú webovú inzertnú platformu pre oblasť služieb a prenájmu. Výsledkom je funkčný ekosystém pozostávajúci zo štyroch samostatných aplikácií: verejnej platformy pre návštevníkov, používateľskej zóny pre registrovaných členov, administračného panela pre správcov a centrálneho REST API servera. Systém je postavený na modernom TypeScript stacku (NestJS, Next.js, PostgreSQL, Prisma ORM) v monorepo architektúre s využitím npm workspaces, čo umožnilo zdieľanie dátových typov a DTO objektov naprieč celým projektom, výrazne zvýšilo stabilitu systému a znížilo počet potenciálnych chýb spôsobených nekonzistentnými dátami. Implementované kľúčové funkcie zahŕňajú bezpečnú autentifikáciu s JWT tokenmi a bcrypt hashovaním hesiel, systém schvaľovania inzerátov s notifikáciami, interaktívnu mapovú vrstvu s Leaflet, dynamické filtre kategórií s JSONB ukladaním v PostgreSQL, vizuálny editor obsahu GrapesJS a analytický subsystém pre sledovanie správania používateľov.

Porovnaním s cieľmi definovanými v úvode práce konštatujem, že všetky stanovené ciele boli splnené. Cieľ vytvorenia stabilného a bezpečného backendového zázemia bol naplnený implementáciou NestJS aplikácie s JWT autentifikáciou, bcrypt hashovaním, globálnou validáciou vstupov a systémom banovania; API je plne zdokumentované cez Swagger. Cieľ troch frontendových aplikácií pre rôzne skupiny používateľov bol naplnený samostatnými aplikáciami \hlcode{platform}, \hlcode{user} a \hlcode{admin}, ktoré poskytujú používateľské rozhranie prispôsobené každej role -- verejná platforma je optimalizovaná pre SEO, administrácia obsahuje CMS s vizuálnym editorom. Cieľ analytických nástrojov a moderácie obsahu bol naplnený asynchrónnym zaznamenávaním kliknutí na inzeráty, grafmi návštevnosti a segmentácie pre administrátora, modulmi reportovania a banovania, ktoré zabezpečujú aktívnu moderáciu komunity.

Testovanie systému prebehlo na viacerých úrovniach -- jednotkové testy biznis služieb (Jest), integračné testovanie REST API endpointov (Swagger, supertest), bezpečnostné testovanie podľa metodiky OWASP Top Ten a manuálne používateľské testovanie kľúčových tokov so zapojením piatich účastníkov. Všetky overené scenáre úspešne potvrdili správnosť implementácie autentifikácie, autorizácie aj biznis logiky. Bezpečnostné opatrenia zahŕňajú XSS a CSRF ochranu, rate limiting prihlasovacích endpointov, audit log kritických akcií a robustný systém banovania účtov.

Platforma tak napĺňa svoju hlavnú misiu: konsolidovať rozptýlenú ponuku lokálnych služieb a prenájmu do jedného transparentného, bezpečného a moderného ekosystému, ktorý odstraňuje nedostatky existujúcich riešení identifikované v analýze súčasného stavu. Ďalší rozvoj platformy môže smerovať k integrácii platobnej brány, presunu médií na externé objektové úložisko, doplneniu reálnych WebSocket notifikácií a integrácii transakčného e-mailového servera, čím sa potenciál systému ďalej rozšíri smerom k plnohodnotnému produkčnému nasadeniu.
